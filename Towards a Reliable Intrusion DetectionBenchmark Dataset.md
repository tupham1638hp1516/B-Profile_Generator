...
>  Dùng `pd.read_csv()` để đọc file. Sau đó tạo một danh sách chứa tên các cột muốn giữ, rồi gọi `df = df[danh_sach_cot]` để loại bỏ tất cả cột thừa. Cuối cùng đổi tên cột sang tên ngắn gọn hơn bằng `df.rename(columns={...})`.
...
>  Trước tiên chuyển cột `timestamp` sang kiểu datetime bằng `pd.to_datetime()`. Sau đó tách riêng thành phần ngày ra thành cột `date` mới (`df['date'] = df['timestamp'].dt.date`). Cuối cùng gọi `df.groupby(['src_ip', 'protocol', 'date'])` — Pandas sẽ tự động tạo ra từng "khối" dữ liệu con độc lập theo 3 tiêu chí này.
...
>  Toàn bộ 4 bước 3.3.x trên có thể gộp thành 1 dòng Pandas: lấy giờ từ timestamp (`dt.hour`), lấy phút (`dt.minute`), tính `(hour * 60 + minute) // 30` rồi gán kết quả vào cột mới `df['bin_index']`. Phép `//` là phép chia lấy phần nguyên, cho ra số từ 0 đến 47.
...
>  Tạo một DataFrame chuẩn gồm đúng 48 hàng, với cột `bin_index` chạy từ 0 đến 47, tất cả giá trị bằng 0. Sau đó dùng `merge` (kiểu `left join`) giữa DataFrame chuẩn này và dữ liệu thực tế. Những bin không có dữ liệu thực sẽ tự động nhận giá trị `NaN`, sau đó gọi `.fillna(0)` để điền 0 vào tất cả.
...
>
> Chi phí tích lũy để đến ô `(i,j)` = Chi phí tại ô đó + Chi phí nhỏ nhất trong 3 ô có thể đến từ đó (trái, dưới, chéo dưới-trái). Sau khi điền đầy ma trận D, giá trị tại ô `D[cuối][cuối]` chính là **Khoảng cách DTW**. Đường đi tối ưu được truy vết ngược từ ô cuối về `(0,0)` bằng cách luôn chọn ô nhỏ nhất trong 3 ô lân cận.
...
>  Dùng thư viện `tslearn`. Gọi `dtw(s1, s2)` để tính khoảng cách giữa 2 mảng. Để áp ràng buộc Sakoe-Chiba, truyền thêm tham số `sakoe_chiba_radius=2`. Để tính toàn bộ ma trận N×N, dùng vòng lặp lồng nhau qua mọi cặp mảng và lưu kết quả vào ma trận 2D.
...
>  Dùng hàm `dtw_barycenter_averaging(sequences)` từ thư viện `tslearn.barycenters`. Đầu vào là một list các mảng 48-phần-tử thuộc cùng một cụm. Hàm này thực hiện toàn bộ vòng lặp Bước 8.2 → 8.3 → 8.4 bên trong và trả về mảng B-Profile cuối cùng.
