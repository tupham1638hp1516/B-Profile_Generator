...
```
[Bước 1] Thu thập mạng (Wireshark / Network Sniffing + MITM)
    ↓  File .PCAP
[Bước 2] CICFlowMeter phân tích PCAP → trích xuất đặc trưng
    ↓  File .CSV (~80 cột / luồng)
[Bước 3 + 4] Python / Pandas → Time-Series Histogram
    ↓  Mảng 48 phần tử cho mỗi [Người dùng – Giao thức – Ngày]
[Bước 5] Zero-filling → Ma trận chuẩn hóa hoàn chỉnh
    ↓  Mảng 48 phần tử đầy đủ (không khoảng trống)
[Bước 6] Thuật toán DTW → Ma trận khoảng cách N×N
    ↓  Số đo độ lệch pha giữa mọi cặp ngày
[Bước 7] X-Means + BIC → Phân cụm tự động
    ↓  Danh sách các cụm hành vi (K tự động tìm)
[Bước 8] DBA → B-Profile
    ↓  1 mảng 48 phần tử đại diện cho mỗi cụm
```

> **Thư viện Python cần thiết:** `pandas`, `numpy`, `tslearn` (DTW + DBA), `pyclustering` (X-Means).
> Cài bằng lệnh: `pip install pandas numpy tslearn pyclustering`
...
**MITM giải quyết vấn đề đó như nào**: Đầu tiên các tác giả/ các tình nguyện viên sẽ đóng vai để thực hiện các thao tác. MITM đứng ở giữa bẻ khóa và nhìn ra các mẫu chuỗi byte đặc biệt. Làm nhiều lần như vậy, ta sẽ rút ra được các chữ ký đại diện cho từng loại hành động. Sau đó, ta đem tập hợp này để dạy cho CICFlowMeter. Nhờ đó mà mặc dù gói tin (do NS bắt) bị khóa một phần, CICFlowMeter vẫn sẽ đủ thông minh để đối chiếu các manh mối với từ điển do MITM cấp, nhận diện và dán nhãn luồng.
...
**Cách dùng CICFlowMeter:** Công cụ này có giao diện đồ họa (GUI) — chỉ cần kéo thả file `.pcap` vào, chọn thư mục đầu ra rồi nhấn "Generate". CICFlowMeter sẽ tự đọc gói tin, gom flow và xuất file `.csv`. Nếu dùng command-line, lệnh cơ bản là: `java -jar CICFlowMeter.jar <input.pcap> <output_dir>`.

**Cấu trúc file CSV đầu ra (các cột quan trọng nhất cho B-Profile):**

| Tên cột | Ý nghĩa |
|---|---|
| `Timestamp` | Thời điểm bắt đầu luồng (dạng `YYYY-MM-DD HH:MM:SS`) |
| `Src IP` | Địa chỉ IP của người dùng (nguồn) |
| `Dst Port` | Cổng đích — dùng để xác định giao thức (80=HTTP, 22=SSH…) |
| `Protocol` | Giao thức (6=TCP, 17=UDP) |
| `Flow Duration` | Thời lượng luồng (`fl_dur`), đơn vị microsecond |
| `Total Fwd Packets` | Tổng số gói tin chiều đi (`tot_fw_pk`) |
| `Total Length of Fwd Packets` | Tổng kích thước gói chiều đi (`tot_l_fw_pkt`), đơn vị byte |

> **Lưu ý ánh xạ tên cột:** Tên cột trong file CSV của CICFlowMeter có thể có khoảng trắng thừa ở đầu (ví dụ `" Flow Duration"`). Cần dùng `.str.strip()` để làm sạch tên cột trước khi xử lý.
...
> 💡 **Hình dung cách code:** Trước tiên chuyển cột `timestamp` sang kiểu datetime bằng `pd.to_datetime()`. Sau đó tách riêng thành phần ngày ra thành cột `date` mới (`df['date'] = df['timestamp'].dt.date`). Cuối cùng gọi `df.groupby(['src_ip', 'protocol', 'date'])` — Pandas sẽ tự động tạo ra từng "khối" dữ liệu con độc lập theo 3 tiêu chí này.
...
> 💡 **Hình dung cách code (Time Binning):** Toàn bộ 4 bước 3.3.x trên có thể gộp thành 1 dòng Pandas: lấy giờ từ timestamp (`dt.hour`), lấy phút (`dt.minute`), tính `(hour * 60 + minute) // 30` rồi gán kết quả vào cột mới `df['bin_index']`. Phép `//` là phép chia lấy phần nguyên, cho ra số từ 0 đến 47.
...
> 💡 **Hình dung cách code (Aggregation):** Sau khi có cột `bin_index`, gọi thêm `.groupby([..., 'bin_index']).agg(...)`. Trong hàm `agg`, truyền vào một dict chỉ định: cột nào thì `'count'` (đếm số luồng), cột nào thì `'sum'` (cộng dồn byte), cột nào thì `'mean'` (lấy trung bình duration). Kết quả là một DataFrame nhỏ gọn với tối đa 48 hàng.
...
**Giai đoạn 1:** Gom nhóm đa chiều (Multi-level Grouping)
...
**Giai đoạn 2:** Ánh xạ Phân vùng Thời gian (Time Binning)
...
**Giai đoạn 3:** Ép nén Dữ liệu (Data Aggregation)
...
> 💡 **Hình dung cách code (Zero-filling):** Tạo một DataFrame chuẩn gồm đúng 48 hàng, với cột `bin_index` chạy từ 0 đến 47, tất cả giá trị bằng 0. Sau đó dùng `merge` (kiểu `left join`) giữa DataFrame chuẩn này và dữ liệu thực tế. Những bin không có dữ liệu thực sẽ tự động nhận giá trị `NaN`, sau đó gọi `.fillna(0)` để điền 0 vào tất cả.
...
> 💡 **Cơ chế Quy hoạch động — Accumulated Cost Matrix:** Để tìm đường đi tối ưu, hệ thống không thử tất cả đường đi có thể (quá chậm). Thay vào đó, nó xây dựng một **Ma trận Chi phí Tích lũy (D)** từng ô một theo công thức:
>
> `D[i][j] = cost[i][j] + min(D[i-1][j], D[i][j-1], D[i-1][j-1])`
>
> Đọc là: Chi phí tích lũy để đến ô `(i,j)` = Chi phí tại ô đó + Chi phí nhỏ nhất trong 3 ô có thể đến từ đó (trái, dưới, chéo dưới-trái). Sau khi điền đầy ma trận D, giá trị tại ô `D[cuối][cuối]` chính là **Khoảng cách DTW**. Đường đi tối ưu được truy vết ngược từ ô cuối về `(0,0)` bằng cách luôn chọn ô nhỏ nhất trong 3 ô lân cận.
...
> 💡 **Hình dung cách code (DTW):** Dùng thư viện `tslearn`. Gọi `dtw(s1, s2)` để tính khoảng cách giữa 2 mảng. Để áp ràng buộc Sakoe-Chiba, truyền thêm tham số `sakoe_chiba_radius=2`. Để tính toàn bộ ma trận N×N, dùng vòng lặp lồng nhau qua mọi cặp mảng và lưu kết quả vào ma trận 2D.
...
> 💡 **Medoid khác Centroid như thế nào?** Trong K-Means thông thường, tâm điểm (Centroid) là điểm ảo tính bằng trung bình cộng tọa độ các thành viên — điểm đó có thể không tồn tại thực trong dữ liệu. K-Medoids (dùng ở đây) thì khác: Medoid **phải là một ngày thực tế có trong dữ liệu**. Cách chọn: với mỗi cụm, lần lượt đặt từng thành viên làm ứng viên Medoid, tính tổng khoảng cách DTW từ ứng viên đó đến tất cả các thành viên còn lại, rồi chọn ứng viên có tổng nhỏ nhất.
...
> 💡 **Công thức BIC (để hiểu rõ hơn):**
>
> `BIC = log-likelihood(data | model) − (k / 2) × log(N)`
>
> Trong đó: `k` = số tham số của mô hình (tỉ lệ với số cụm), `N` = tổng số điểm dữ liệu. Số hạng đầu (`log-likelihood`) là **điểm thưởng** — dữ liệu càng co cụm chặt thì điểm càng cao. Số hạng sau là **điểm phạt** — càng nhiều cụm thì bị phạt càng nặng. BIC cao nghĩa là mô hình giải thích dữ liệu tốt mà không quá phức tạp.
...
> 💡 **Hình dung cách code (X-Means):** Dùng thư viện `pyclustering`. Đầu vào là ma trận khoảng cách DTW N×N (đã tính ở Bước 6). Khởi tạo 2 tâm điểm ban đầu bằng `kmeans_plusplus_initializer`, sau đó tạo đối tượng `xmeans` với `kmax` là giới hạn số cụm tối đa. Gọi `.process()` để chạy, rồi `.get_clusters()` để lấy danh sách các cụm. Mỗi cụm là một list chứa chỉ số (index) của các ngày thuộc cụm đó.
...
- *Ví dụ (C_tmp = Ngày 1, xử lý Ngày 2 và Ngày 5 lần lượt):*
  - **Ngày 2:** đỉnh **45** tại Index 2 (lướt web trễ 30 phút). Warping Path phát hiện đây khớp với **Index 1 của `C_tmp`** — ghi lại ánh xạ `(Index_2_Ngày2 → Index_1_Ctmp)`.
  - **Ngày 5:** đỉnh **48** tại Index 1 (cùng giờ với C_tmp). Warping Path phát hiện đây khớp thẳng với **Index 1 của `C_tmp`** — ghi lại ánh xạ `(Index_1_Ngày5 → Index_1_Ctmp)`.
  - Kết thúc Bước 8.2: mỗi cột của `C_tmp` đã có một danh sách các giá trị được chỉ định về nó từ toàn bộ Ngày 2 và Ngày 5.
...
- *Ví dụ (3 ngày):*
  - Cột Index 1 của `C_tmp` được ánh xạ tới:
    - Ngày 1: giá trị **50** (tại Index 1)
    - Ngày 2: giá trị **45** (tại Index 2 — lệch pha, nhưng DTW đã căn chỉnh)
    - Ngày 5: giá trị **48** (tại Index 1)
  - Giá trị mới tại Cột Index 1 của `C_tmp`: `(50 + 45 + 48) / 3 = 47.67`.

  > Khi cụm có nhiều chuỗi, hệ thống vẫn ánh xạ từng ngày một rồi **gom tất cả giá trị được chỉ định về cùng một cột**, sau đó tính trung bình cộng của tất cả chúng.
...
> 💡 **Hình dung cách code (DBA):** Dùng hàm `dtw_barycenter_averaging(sequences)` từ thư viện `tslearn.barycenters`. Đầu vào là một list các mảng 48-phần-tử thuộc cùng một cụm. Hàm này thực hiện toàn bộ vòng lặp Bước 8.2 → 8.3 → 8.4 bên trong và trả về mảng B-Profile cuối cùng.
...
|---|---|
| **PCAP** | Packet Capture — định dạng file lưu trữ các gói tin mạng thô, bắt bằng Wireshark hoặc tcpdump |
| **Flow (Luồng mạng)** | Tập hợp tất cả các gói tin thuộc cùng một phiên kết nối, xác định bởi 5-tuple |
| **5-tuple** | Bộ 5 giá trị định danh một flow: IP nguồn, Cổng nguồn, IP đích, Cổng đích, Giao thức |
| **Bin / Time Bin** | Một khoảng thời gian 30 phút trong ngày; toàn ngày có 48 bin đánh số từ 0 đến 47 |
| **Centroid** | Tâm điểm ảo trong K-Means, tính bằng trung bình tọa độ các thành viên — có thể không tồn tại trong dữ liệu thực |
| **Medoid** | Tâm điểm thực tế trong K-Medoids, **phải là một điểm có trong dữ liệu**, được chọn là điểm có tổng khoảng cách đến các thành viên khác nhỏ nhất |
| **DTW** | Dynamic Time Warping — thuật toán đo khoảng cách giữa 2 chuỗi thời gian có khả năng bù trừ lệch pha |
| **Accumulated Cost Matrix** | Ma trận D trong DTW, mỗi ô `D[i][j]` chứa chi phí tích lũy nhỏ nhất để căn chỉnh đến điểm `(i,j)` |
| **Warping Path** | Đường đi tối ưu qua ma trận DTW, chỉ ra cách ghép cặp các điểm của 2 chuỗi |
| **Sakoe-Chiba Band** | Ràng buộc cửa sổ trong DTW, giới hạn độ lệch pha tối đa giữa 2 chuỗi |
| **BIC** | Bayesian Information Criterion — tiêu chuẩn chấm điểm mô hình, cân bằng giữa độ khớp dữ liệu và độ phức tạp |
| **X-Means** | Phiên bản mở rộng của K-Means, tự động tìm số cụm K tối ưu bằng cách thử chẻ đôi và so sánh BIC |
| **DBA** | DTW Barycenter Averaging — thuật toán tính trung bình chuỗi thời gian có bù lệch pha, tránh làm phẳng các đỉnh |
| **B-Profile** | Benign Profile — mảng 48 phần tử đại diện cho một cụm hành vi lành tính, đầu ra cuối cùng của toàn bộ pipeline |
